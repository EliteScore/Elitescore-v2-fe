    ...
    <div className="glass-card rounded-2xl border border-[#2563eb]/20 bg-gradient-to-br from-card/80 to-background backdrop-blur-md p-6 shadow-xl relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {skills.map((skill) => (
          <div
            key={skill.name}
            className="glass-card rounded-xl border border-white/5 bg-card/30 backdrop-blur-sm p-4 hover:border-[#2563eb]/30 transition-all"
          >
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{skill.name}</span>
              <span className="text-sm font-bold text-[#2563eb]">{skill.progress}%</span>
            </div>
